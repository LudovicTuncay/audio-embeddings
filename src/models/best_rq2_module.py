...
class BestRQ2Module(L.LightningModule):
    ...
    def compute_encoder(self, patches: torch.Tensor, mask: torch.Tensor, grid_size: Tuple[int, int]) -> torch.Tensor:
        B, N, _ = patches.shape
        m = mask[0] # [N]
    ...
    def compute_predictor(self, encoder_out: torch.Tensor, mask: torch.Tensor, grid_size: Tuple[int, int]) -> torch.Tensor:
        ...
        encoder_out_proj = self.predictor_input_proj(encoder_out) # [B, N_keep, pred_dim]
